...
export const fetchPostCommentsByQuestionId: TFetchPostCommentsByQuestionId = async questionId => {
  const data: ApiResponse<Comment> = await fetch(
    `${BASE_URL}/questions/${questionId}/comments?order=desc&site=stackoverflow&filter=withbody`
  ).then(res => res.json())
...
export const fetchAnswerCommentsByAnswerId: TFetchAnswerCommentsByAnswerId = async answerId => {
  const data: ApiResponse<Comment> = await fetch(
    `${BASE_URL}/answers/${answerId}/comments?order=desc&sort=creation&site=stackoverflow&filter=withbody`
  ).then(res => res.json())
