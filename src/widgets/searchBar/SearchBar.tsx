...
export const SearchBar = () => {
  ...
  // @ts-ignore
  const { isPending, data: questions, refetch } = useQuery({
    queryKey: ['questions'],
  ...
  return (
    <div className="flex flex-col gap-10 items-center m-auto">
      ...
      <div className="grid md:grid-cols-4 gap-4 grid-flow-cols md:max-w-[80%]">
        {isPending && (
          (new Array(3)).map((_) => <QuestionCardSkeleton />)
        )}
        {!isPending && questions && questions.length > 0 && questions.map(questionProps => (
          <QuestionCard key={questionProps.creation_date} {...questionProps} />
        ))}
      </div>
    </div>
  )
